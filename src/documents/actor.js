...
export default class ActorWFRP4e extends WarhammerActor
{
  ...
  {
      let test = await this._setupTest(ChannellingDialog, ChannelTest, spell, context, options, false);
  ...
  async setupDispel(test)
  {
    ...
    if (!skill)
    {
      throw new Error(`${this.name} nie posiada Języka (Magiczny)!`)
    }

    return  this.setupSkill(skill.name, {dispel: test.message.id, appendTitle: ` - Rozproszenie`});
  }
