export default class ValueDialog extends Dialog 
{
    ...
    static create(text, title, defaultValue = "", values={})
    {
        ...
        `<div class="value-dialog"><p>${text || "Wprowadź Wartość"}</p><input class="value" type="text" value="${defaultValue}"></div>` 
        : 
        `<div class="value-dialog"><p>${text || "Wybierz Wartość"}</p><select class="value" value="${defaultValue}"><option value=""></option>${Object.keys(values).map(
            v => `<option value=${v}>
